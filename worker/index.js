...
async function getCategories(env) {
  ...
  // NOTE: We only need minimal fields. We’ll fetch root categories and one level is usually enough for MVP.
  // If you want deeper: we can paginate / traverse later.
  ...
  for (const c of allCats) {
    ...
    if (name.toLowerCase().includes(needle)) {
      out.push({
        id: c.id,
        name,
        // Ecwid category may include a `url` in some setups; if missing, frontend can build a fallback.
        url: c.url || null,
      });
...
function trimProducts(items = []) {
  return items.map((p) => {
    ...
    // Prefer cleanUrls if enabled on request; Ecwid may return `url` or `cleanUrl`.
...
async function handleSearch(request, env) {
  ...
  // Ecwid keyword: add * to avoid exact-match behavior.
  const keyword = encodeURIComponent(`${q}*`);

  // Tight, storefront-like search for typeahead
  const params = [
    `keyword=${keyword}`,
